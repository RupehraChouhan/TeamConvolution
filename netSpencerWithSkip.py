...
def net(input, is_training, dropout_kept_prob):
	...
	conv4   = tf.nn.conv2d(pool3, conv4_W, strides=[1, 1, 1, 1], padding='SAME') + conv4_b
	drop = tf.layers.dropout(conv4,rate=1-dropout_kept_prob, noise_shape=None, seed=None, training=is_training)
	norm = batch_norm(drop, is_training, name="batch_norm4")
	activation = tf.nn.relu(norm)

	# Layer 3: Convolutional. Input = 24x24x16. Output = 20x20x24.
	#conv3_W = tf.Variable(tf.truncated_normal(shape=(5, 5, 16, 6), mean = mu, stddev = sigma), name='conv3_W')
	conv5_W = tf.get_variable("conv5_W", shape=[1, 1, 64, 64], initializer=xavier)
	conv5_b = tf.Variable(tf.zeros(64), name='conv5_b')
	conv5   = tf.nn.conv2d(activation, conv5_W, strides=[1, 1, 1, 1], padding='SAME') + conv5_b
	drop = tf.layers.dropout(conv5,rate=1-dropout_kept_prob, noise_shape=None, seed=None, training=is_training)
	norm = batch_norm(drop, is_training, name="batch_norm5")
	activation = tf.nn.relu(norm)

	# Skip
	skip = pool3 + activation;
	...
	fc0   = tf.contrib.layers.flatten(skip)
